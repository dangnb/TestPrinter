...
class _PrintingProgressDialogState extends State<PrintingProgressDialog> {
  ...
  Future<void> print(String divice) async {
    ...
    if (imagePrint != null) {
      ...
      if (imagePrint.length > space) {
        ...
        while (index < count.toInt()) {
          ...
          queue.add(Uint8List.fromList(byte));
          index++;
        }
        bytes = [];
      } else {
      ...
      bytes += generator.cut();
      queue.add(Uint8List.fromList(bytes));
    }

    while (queue.isNotEmpty) {
      var uint8List = queue.removeFirst();
